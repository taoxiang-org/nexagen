...
Extract all MCP agents from the provided `mcp_cards.json` and transform them into the A2A standard agent cards format. Return the result as a JSON array.
...
# Output Format
The output should be a JSON array where each object represents an agent card in the A2A standard format. Each agent card should include the following fields:
...
This prompt ensures that MCP tools are correctly mapped to the skills in the A2A format.
